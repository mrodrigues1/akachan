...
class GetFeedingFrequencyTrendUseCaseTest {
    ...
    private val useCase = GetFeedingFrequencyTrendUseCase(clock)

    @Test
    fun `counts breastfeeding and bottle feeds per day with zero-fill`() = runTest {
        val feeds = TrendFeedInstants(
            breast = listOf(Instant.parse("2026-06-14T08:00:00Z"), Instant.parse("2026-06-14T11:00:00Z")),
            bottle = listOf(Instant.parse("2026-06-13T09:00:00Z")),
        )

        val result = useCase(TrendRange.SEVEN_DAYS, feeds)
    ...
    fun `buckets near-midnight feeds by the clock's local day`() = runTest {
        ...
        val zonedUseCase = GetFeedingFrequencyTrendUseCase(saoPauloClock)
        val feeds = TrendFeedInstants(breast = listOf(Instant.parse("2026-06-14T02:00:00Z")))

        val result = zonedUseCase(TrendRange.SEVEN_DAYS, feeds)
