...
class GetDayRhythmTrendUseCase @Inject constructor(
    ...
    suspend operator fun invoke(range: TrendRange, feeds: TrendFeedInstants): List<DayRhythm> =
        withContext(Dispatchers.Default) {
            ...
            val breastByDate = feeds.breast.groupBy { it.atZone(zone).toLocalDate() }
            val bottleByDate = feeds.bottle.groupBy { it.atZone(zone).toLocalDate() }
